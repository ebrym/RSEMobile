...
import com.redstar.rsemobile.Global;
import com.redstar.rsemobile.dataList;
...
public class DataDB {
	...
	// chemonics DB Methods
    public boolean checkCHMAWBNOPOD(Context context,String AWBNO){
        con = new Connection(context);
        ...
        if(con.checkDataBase()){
            con.openDataBase();
            SQLiteDatabase db = con.getWritableDatabase();
            Cursor cursor = db.rawQuery("SELECT * FROM ChemonicsPOD WHERE WayBillNumber='" + AWBNO + "'", null);
    ...
    public Cursor getCHMPODNotUploaded(Context context){
        con = new Connection(context);
        ...
        SQLiteDatabase db = con.getWritableDatabase();
        Cursor dcursor = db.rawQuery("SELECT * FROM ChemonicsPOD WHERE TransferStatus='N'", null);
        return dcursor;

    }
    public String getAvailableCHMPODCount(Context context){
        con = new Connection(context);
        String CHMPODCount = "";
        try {
            con.createDataBase();
        ...
        if(con.checkDataBase()){
            con.openDataBase();
            SQLiteDatabase db = con.getWritableDatabase();
            Cursor cursor = db.rawQuery("SELECT count(*) as CHMPODCount FROM ChemonicsPOD WHERE TransferStatus='N'", null);
            ...
            if (cursor.getCount() < 1) // No scans to upload
            {
                ...
                if (cursor.moveToFirst()) {
                    CHMPODCount = cursor.getString(cursor.getColumnIndex("CHMPODCount"));
